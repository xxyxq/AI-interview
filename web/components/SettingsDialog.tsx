                            ...
                            {config.models.length > 0 && (
                                <>
                                    ...
                                    <div className="p-3 bg-purple-50 rounded-lg border border-purple-100">
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-xs text-gray-600">语音模型（需选择"支持音频输入输出"的 Omni 多模态模型）</label>
                                            <div className="relative">
                                                <select
                                                ...
                                                >
                                                    <option value="">选择模型</option>
                                                    {config.models
                                                        // 与 store hydrate 共用同一个过滤口径 isVoiceCandidateModel，
                                                        // 保证下拉里一定能看到 hydrate 自动注入的 DeepSeek-V4-Flash（语音测试）
                                                        .filter(isVoiceCandidateModel)
                                                        .map((model) => (
                                                            <option key={model.id} value={model.id}>
                                                                {model.name}
                                                            </option>
                                            ...
                                            <p className="text-xs text-purple-700 mt-1.5 flex items-start gap-1.5 leading-relaxed">
                                                <AlertCircle className="w-3 h-3 mt-0.5 flex-shrink-0" />
                                                <span>
                                                    已支持的语音模型：阿里云百炼 <code className="bg-white/60 px-1 rounded">qwen3-omni-*</code> /
                                                    &nbsp;DeepSeek <code className="bg-white/60 px-1 rounded">deepseek-v4-pro / deepseek-v4-flash</code> /
                                                    &nbsp;其它兼容平台的 audio omni 模型。
                                                    普通纯文本模型（如 deepseek-chat / qwen3-max / glm-5）不支持语音，请勿选择。
                                                </span>
                                            </p>

                                            {/* 两个一键按钮 —— 为了让你直接"选上去能测 deepseek-v4-flash" */}
                                            <div className="pt-1 flex flex-col sm:flex-row gap-2">
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        // 先看是否已存在同名模板；如果没有就新建，有就直接绑定
                                                        const existing = config.models.find(
                                                            m =>
                                                                m.model === "deepseek-v4-flash" &&
                                                                (m.baseUrl || "").includes("deepseek.com")
                                                        );
                                                        if (existing) {
                                                            onSetVoiceModel(existing.id);
                                                            toast.success(
                                                                `已切换为语音模型：${existing.name}。如 API Key 为空，请点击右侧"补填 Key"。`
                                                            );
                                                            return;
                                                        }
                                                        // 优先复用现有 DeepSeek 通道的 API Key，省得重复填
                                                        const sibling = config.models.find(m =>
                                                        ...
                                                        if (newModel) {
                                                            onSetVoiceModel(newModel.id);
                                                            toast.success(
                                                                sibling?.apiKey
                                                                    ? "已自动生成 DeepSeek-V4-Flash 语音配置并复用了 API Key，可以直接开始测试。"
                                                                    : "已自动生成 DeepSeek-V4-Flash（语音测试）。点击下面的「补填 API Key」粘贴你的密钥。",
                                                                { duration: 5000 }
                                                            );
                                                        }
                                                    }}
                                                    className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg
                                                               ...
                                                               hover:from-purple-700 hover:to-indigo-700 transition-all active:scale-[0.98]"
                                                >
                                                    <Plus className="w-3.5 h-3.5" />
                                                    一键生成 DeepSeek-V4-Flash（语音测试）
                                                </button>
                                                {(() => {
                                                    const cur = config.models.find(m => m.id === config.voiceModelId) || undefined;
                                                    const disabled = !cur;
                                                    return (
                                                        <button
                                                            type="button"
                                                            disabled={disabled}
                                                            title={disabled ? "请先选择或生成一个语音模型" : "打开编辑窗口，粘贴你的 DeepSeek API Key 后保存"}
                                                            onClick={() => {
                                                                if (!cur) return;
                                                                // 直接利用 SettingsDialog 已经有的编辑弹窗机制
                                                                setEditingModel(cur);
                                                                setSourceModel(undefined);
                                                                setShowModelForm(true);
                                                                // 顺手弹一个 toast 提醒用户这次进来就是专门补 Key 的
                                                                setTimeout(() => {
                                                                    toast("补填 API Key", {
                                                                        description:
                                                                            "把你的 DeepSeek API Key 粘到「API Key」输入框里，点保存即可开始测试语音。",
                                                                        duration: 6000,
                                                                    });
                                                                }, 150);
                                                            }}
                                                            className={cn(
                                                                "flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border shadow-sm transition-all",
                                                                disabled
                                                                    ? "border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed"
                                                                    : "border-purple-200 bg-white text-purple-700 hover:bg-purple-50 active:scale-[0.98]"
                                                            )}
                                                        >
